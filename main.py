...
def page_rank(G: nx.DiGraph):
    ...
    dangling_nodes = [int(node) for node in G.nodes if G.out_degree(node) == 0] #List of nodes with no outgoing edges
    ...
    print("Sx_k")
    print(Sx_k)
    for k in range(3): #outer loop. Multiply current vector by M until eigenvector is reached
        ...
        for i in range(n): #loop over all nodes
            #get links to i
            links_to_i = list(rev_G.successors(str(i))) #using links to i, since A is sparse aka all other entries of A is 0. So matrix multiplication is too performance expensive
            ak_sum = sum(1/n * x_k[int(j)] for j in links_to_i)
            M = (1-m)*ak_sum+(1-m)*dangling_sum+m*Sx_k
            x_k_1 = [score * M for score in x_k]
            print("***** PAGERANK******")
            print(x_k_1)
            x_k = x_k_1
